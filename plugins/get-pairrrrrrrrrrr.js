...
cmd({
    ...
    react: "📉",
    desc: "Get pairing code for.....",
    category: "download",
    use: ".pair 92323XXX",
    filename: __filename
}, async (conn, mek, m, { from, quoted, body, isCmd, command, args, q, isGroup, senderNumber, reply, config }) => {
    try {
        // Check if in group
        if (isGroup) {
        ...
        const sentMessage = await conn.sendMessage(from, {
            image: { url: config?.BOT_PP || config?.IMAGE_PATH },
