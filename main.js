...
const globalRestartAttempts = new Map(); // numéro -> nombre de tentatives (persiste entre les appels à arslanPair)
...
function isNumberAlreadyConnected(number) {
    ...
    // Un socket peut exister sans être réellement connecté (code de pairing
    // pas encore validé, ou expiré). On ne bloque les nouvelles tentatives
    // que si la session est vraiment enregistrée auprès de WhatsApp.
    return !!(conn && conn.authState?.creds?.registered);
}
...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [𝐀𝐍𝐎𝐒_𝐕𝐎𝐋𝐃𝐈𝐆𝐎𝐀𝐃_𝐌𝐈𝐍𝐈_𝐌𝐃] ${new Date().toISOString()}: ${message}`);
}
...
function setupAutoRestart(socket, number) {
    ...
    if (!globalRestartAttempts.has(sanitized)) globalRestartAttempts.set(sanitized, 0);
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            const attempts = globalRestartAttempts.get(sanitized) || 0;
            if (attempts < maxRestartAttempts) {
                globalRestartAttempts.set(sanitized, attempts + 1);
                arslanLog(`Reconnecting ${number} (${attempts + 1}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                arslanLog(`Max restart attempts reached for ${number}. Session considérée morte — nettoyage.`, 'error');
                globalRestartAttempts.delete(sanitized);
                // Sans ça, le numéro restait marqué "connecté" indéfiniment
                // et bloquait toute nouvelle tentative de pairing (SERVER FULL).
                activeSockets.delete(sanitized);
                socketCreationTime.delete(sanitized);
                // 3 échecs de reconnexion d'affilée = session cassée/déconnectée
                // pour de bon (banni, conflit, session invalide...). On la
                // supprime de MongoDB pour ne pas la retenter à chaque
                // redémarrage du serveur et libérer l'espace en base.
                try {
                    await deleteSessionFromMongoDB(sanitized);
                    await removeNumberFromMongoDB(sanitized);
                    arslanLog(`Session MongoDB supprimée pour ${sanitized}`, 'info');
                } catch (e) {
                    arslanLog(`Échec suppression session MongoDB pour ${sanitized}: ${e.message}`, 'error');
                }
            }
        }
        if (connection === 'open') { globalRestartAttempts.delete(sanitized); }
    });
}
...
async function arslanPair(number, res = null) {
    ...
    try {
        ...
        // Un ancien socket non enregistré (pairing précédent jamais terminé)
        // peut encore traîner dans activeSockets : on le ferme proprement
        // avant de repartir sur une nouvelle tentative pour ce numéro.
        const staleConn = activeSockets.get(sanitizedNumber);
        if (staleConn) {
            try { staleConn.ev.removeAllListeners(); staleConn.end?.(new Error('Superseded by new pairing attempt')); } catch (e) {}
            activeSockets.delete(sanitizedNumber);
            socketCreationTime.delete(sanitizedNumber);
        }
        ...
        // Une session existait dans MongoDB mais n'a jamais fini de s'enregistrer
        // (creds.registered === false) : c'est un reliquat d'un pairing qui a
        // échoué en cours de route. La réutiliser telle quelle fait planter
        // requestPairingCode() avec "Cannot read properties of undefined
        // (reading 'public')" et boucle indéfiniment. On la purge et on repart
        // sur un état d'authentification 100% neuf.
        if (existingSession && !state.creds.registered) {
            arslanLog(`Session incomplète/corrompue pour ${sanitizedNumber} (jamais finalisée) — réinitialisation`, 'warning');
            await fs.remove(sessionPath);
            await deleteSessionFromMongoDB(sanitizedNumber);
            fs.ensureDirSync(sessionPath);
        ...
        // Sans ça, Baileys utilise une version de protocole WhatsApp Web
        // figée/obsolète, et WhatsApp ferme la connexion immédiatement avec
        // "405 - Connection Failure" avant même que le pairing puisse marcher.
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: '𝐀𝐍𝐎𝐒_𝐕𝐎𝐋𝐃𝐈𝐆𝐎𝐀𝐃_𝐌𝐈𝐍𝐈_𝐌𝐃' };
            }
        });
        ...
        if (!conn.authState.creds.registered) {
            ...
            try {
                ...
                // Un code de pairing WhatsApp expire après ~60s. S'il n'est
                // jamais validé, on nettoie ce socket pour que la prochaine
                // tentative reparte sur une base propre (sinon: "SERVER FULL").
                setTimeout(() => {
                    const c = activeSockets.get(sanitizedNumber);
                    if (c && c === conn && !conn.authState.creds.registered) {
                        arslanLog(`Pairing code for ${sanitizedNumber} expired without being used — cleaning up`, 'warning');
                        try { conn.ev.removeAllListeners(); conn.end?.(new Error('Pairing code expired')); } catch (e) {}
                        activeSockets.delete(sanitizedNumber);
                        socketCreationTime.delete(sanitizedNumber);
                        globalRestartAttempts.delete(sanitizedNumber);
                    }
                }, 90000);
            } catch (error) {
    ...
    try {
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    ...
                    const welcomeBotName = welcomeConfig.BOT_NAME || '𝐀𝐍𝐎𝐒_𝐕𝐎𝐋𝐃𝐈𝐆𝐎𝐀𝐃_𝐌𝐈𝐍𝐈_𝐌𝐃';
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363422530463919@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
                ...
                const displayBotName = effectiveConfig.BOT_NAME || 'ARSLAN-MD';
                const myquoted = {
        ...
        arslanLog(`𝐀𝐍𝐎𝐒_𝐕𝐎𝐋𝐃𝐈𝐆𝐎𝐀𝐃_𝐌𝐈𝐍𝐈_𝐌𝐃 Pair error: ${err.message}`, 'error');
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'anos_voldigoad_mini_md is running 🔥', activeSessions: activeSockets.size }));
router.get('/connect-all', async (req, res) => {
    try {
        ...
        for (const number of numbers) {
            ...
            const mockRes = { headersSent: false, json: () => {}, send: () => {}, status: () => mockRes };
            await arslanPair(number, mockRes);
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 𝐀𝐍𝐎𝐒_𝐕𝐎𝐋𝐃𝐈𝐆𝐎𝐀𝐃_𝐌𝐈𝐍𝐈_𝐌𝐃 — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
